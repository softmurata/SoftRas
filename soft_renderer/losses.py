...
class TexLaplacianLoss(nn.Module):
    def __init__(self, faces, average=False):
        ...
        self.nf = faces.size(0)
        self.average = average
        texlaplacian = np.zeros([self.nf, self.nf]).astype(np.float32)

        edge2face = self.getEdge2Faces(faces)
        texlaplacian[edge2faces[:, 0], edge2faces[:, 1]] = -1
        texlaplacian[edge2faces[:, 1], edge2faces[:, 0]] = -1

        r, c = np.diag_indices(texlaplacian.shape[0])
        laplacian[r, c] = -texlaplacian.sum(1)

        for i in range(self.nf):
            texlaplacian[i, :] /= texlaplacian[i, i]

        self.register_buffer('tex_laplacian', torch.from_numpy(texlaplacian))
        ...
        for i, f in enumerate(faces_sorted):
            ...
            for e in edges:
                ...
                if edge2face[edge2idx[e], 0] == -1:
                    ...
                    print(e, ':This edge has more than two faces')
    ...
    def forward(self, x):
        batch_size = x.size(0)
        x = torch.matmul(self.texlaplacian, x)
        dims = tuple(range(x.ndimension())[1:])
        x = x.pow(2).sum(dims)
        if self.average:
            return x.sum() / batch_size
