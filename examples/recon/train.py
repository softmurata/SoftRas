...
import torch
import numpy as np
from losses import multiview_iou_loss, multiview_color_loss
...
def train():
    ...
    for i in range(start_iter, args.num_iterations + 1):
        ...
        tex_laplacian_loss = tex_laplcian_loss.mean()
        # multiview color loss
        color_loss = 0
        if tex_laplcian_loss != 0:
        ...
        # compute loss
        loss = iou_loss + args.lambda_laplacian * laplacian_loss +
        args.lambda_laplacian * tex_laplcian_loss + args.lambda_flatten * flatten_loss
        + color_loss
