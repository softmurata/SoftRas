...
class Encoder(nn.Module):
    def __init__(self, dim_in=4, dim_out=1024, dim1=64, dim2=2048, im_size=512):
        super(Encoder, self).__init__()

        dim_hidden = [dim1, dim1 * 2, dim1 * (2**2), dim1 * (2**3), dim2, dim2]
        ...
        self.conv4 = nn.Conv2d(dim_hidden[2], dim_hidden[3], kernel_size=5, stride=2, padding=2)

        self.bn1 = nn.BatchNorm2d(dim_hidden[0])
        self.bn2 = nn.BatchNorm2d(dim_hidden[1])
        self.bn3 = nn.BatchNorm2d(dim_hidden[2])
        self.bn4 = nn.BatchNorm2d(dim_hidden[3])

        self.fc1 = nn.Linear(dim_hidden[3] * math.ceil(im_size/2**4) ** 2, dim_hidden[4])
        self.fc2 = nn.Linear(dim_hidden[4], dim_hidden[5])
        self.fc3 = nn.Linear(dim_hidden[5], dim_out)
...
class Decoder(nn.Module):
    ...
    def __init__(self, filename_obj, dim_in=1024, centroid_scale=0.1, bias_scale=1.0, centroid_lr=0.1, bias_lr=1.0):
        ...
        self.fc_centroid = nn.Linear(dim_hidden[2], 3)  # (x, y, z)
        self.fc_bias = nn.Linear(dim_hidden[2], self.nv * 3)
...
class Model(nn.Module):
    def __init__(self, filename_obj, args):
        ...
        self.viewing_angle = 360 / self.num_views
        ...
        self.renderer = sr.SoftRenderer(image_size=args.image_size, sigma_val=args.sigma_val,
                                        aggr_func_rgb='hard', camera_mode='look_at', viewing_angle=self.viewing_angle,
                                        dist_eps=1e-10)

        self.gen_tex = False  # Do you want to generate textures?
        self.col_gen = ColorGen(filename_obj)

        # to construct loss
        self.tex_laplacian_loss = sr.TexLaplacianLoss(self.decoder.faces)  # texture loss
    ...
    def predict_multiview(self, image_a, image_b, viewpoint_a, viewpoint_b):
        ...
        silhouettes = self.renderer(vertices, faces)  # softrenderer(softrasterizer) forward,  where is textures?
        return silhouettes.chunk(4, dim=0), laplacian_loss, flatten_loss, tex_laplacian_loss

    def evaluate_iou(self, images, voxels):
        vertices, faces = self.reconstruct(images)

        faces_ = srf.face_vertices(vertices, faces).data
        faces_norm = faces_ * 1. * (self.voxel_size - 1) / self.voxel_size + 0.5  # 32 is voxel_size
        voxels_predict = srf.voxelization(faces_norm, self.voxel_size, False).cpu().numpy()
