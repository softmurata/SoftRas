...
# CLASS_IDS_ALL = ('0')  # 64 * 64, 128 * 128, 256 * 256
# CLASS_IDS_ALL = ('1')  # 384 * 384
CLASS_IDS_ALL = ('2')  # 640 * 640
...
MODEL_DIRECTORY = 'mlresults/models'  # save weight parameters
...
MODEL_TYPE = 'normal'  # 'normal' or 'new'
MULTI = False
...
parser.add_argument('-vs', '--voxel_size', type=int, default=VOXEL_SIZE)
parser.add_argument('-b', '--batch-size', type=int, default=BATCH_SIZE)
parser.add_argument('-mt', '--model_type', type=str, default=MODEL_TYPE)
parser.add_argument('-vp', '--view_point', type=int, default=NUM_VIEWS)
...
if args.model_type == 'normal':
    ...
    model = models_new.Model('mldataset/templatemesh/sphere/sphere_642.obj', args=args)
...
dataset_train = datasets.MonoShapeNet(args, args.dataset_directory, args.class_ids.split(','), 'train')
...
def train():
    ...
    for i in range(start_iter, args.num_iterations + 1):
        ...
        # soft render images
        render_images, laplacian_loss, flatten_loss, tex_laplacian_loss = model([images_a, images_b],
                                                            [viewpoints_a, viewpoints_b],
                                                            task='train')
        laplacian_loss = laplacian_loss.mean()
        flatten_loss = flatten_loss.mean()
        if tex_laplacian_loss is None:
            tex_laplacian_loss = 0
        else:
            tex_laplacian_loss = tex_laplacian_loss.mean()
        # multiview color loss
        color_loss = 0
        if tex_laplacian_loss is not None:
            color_loss = multiview_color_loss(render_images, images_a, images_b)

        # multiview iou loss
        iou_loss = multiview_iou_loss(render_images, images_a, images_b)

        # compute loss
        loss = iou_loss + args.lambda_laplacian * laplacian_loss + args.lambda_laplacian * tex_laplacian_loss + args.lambda_flatten * flatten_loss + color_loss
        ...
        if i % args.demo_freq == 0:
            ...
            if demo_t is None:
                ...
                textures = demo_t.clone().cuda().data.cpu().numpy()
