...
MODEL_DIRECTORY = 'mlresults/models/recon/checkpoint_0200000.pth.tar'
...
state_dicts = torch.load(args.model_directory)  # after learning, model_directory creates ./data/results/models/
model.load_state_dict(state_dicts['model'], strict=False)
model.eval()

# create dataset for validation
dataset_val = datasets.MonoShapeNet(args, args.dataset_directory, args.class_ids.split(','), 'val')

directory_output = './mldataset/results/test'
...
def test():
    ...
    for class_id, class_name in dataset_val.class_ids_pair:
        ...
        for i, (im, vx) in enumerate(dataset_val.get_all_batches_for_evaluation(args.batch_size, class_id)):
            images = torch.autograd.Variable(im).cuda()  # (batch_size, 4, image_size, image_size)
            voxels = vx.numpy()
            ...
            for k in range(vertices.size(0)):
                ...
                if obj_id % args.save_freq == 0:
                    ...
                    srf.save_obj(mesh_path, vertices[k], faces[k])  # create mesh
                    imageio.imsave(input_path, img_cvt(images[k]))  # create video
